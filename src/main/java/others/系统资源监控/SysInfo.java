        	...
        	new Thread(new Runnable() {
				public void run() {
					...
					for (int j=0;j<1000;j++) {
						//System.out.println("IP :"+sysInfo.getDefaultIpAddress());
				        try {
				        	
							//System.out.println("Thread ID " + j + " CPU Idle:" + CpuPerc.format(sigar.getCpuPerc().getIdle()));
				        	//CpuPerc.format(sigar.getCpuPerc().getIdle());
							try {
								long start = System.currentTimeMillis();
								String str = CpuPerc.format(cpu.getIdle());
								String str1 = CpuPerc.format((float)sigar.getMem().getFree());
								long end = System.currentTimeMillis();
								System.out.println("cost time: " + (end-start) + "  data:"+str+str1);
								Thread.sleep(200);
